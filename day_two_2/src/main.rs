...
fn main() {
    ...
    let numbers = input
        ...
        .collect::<Vec<usize>>();
    let mut new_codes = numbers;
    for first in 1..100 {
        if let Some(elem) = new_codes.get_mut(1) {
            *elem = first;
        }
        for second in 1..100 {
            if let Some(elem) = new_codes.get_mut(2) {
                *elem = second;
            }
            if intcode_program(&new_codes) == 19_690_720 {
                println!(
                    "{} and {}",
                    new_codes.get(1).unwrap(),
                    new_codes.get(2).unwrap()
                );
                return;
            }
        }
